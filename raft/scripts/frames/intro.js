...
define([], function () {
    return function (frame) {
        ...
        .after(600, function () {
            frame.model().subtitle = '<h3>在这个例子中, 你可以将这个 <span style="color:steelblue">节点</span> 看作是存储值的数据服务器。</h3>'
                           + frame.model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
        ...
        .after(1000, function () {
            frame.model().subtitle = '<h3>我们还有一个<span style="color:green">客户端</span>，可以发送值到这个服务器</h3>'
                           + frame.model().controls.html();
            layout.invalidate();
        })
        .after(500, function () {
        ...
        .after(100, wait).indefinite()
        .after(100, function () {
            frame.model().subtitle = '<h3>一个节点就很容易达成 <em> 共识 </em> </h3>'
                           + frame.model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
